...
-   `component` **([Function][71] \| [Object][72])?** A custom modal component to use to wrap your component. By default, `Modal` from [`react-modal`][74] will be used. This wrapper is passed the following props: `{ warning, disableOutsideClick, show, handleHide, children }`. If `null` is provided, no wrapper will be used.
-   `warning` **[Boolean][75]** A boolean representing whether to add the `modal-warning` class to the surrounding `div`. (optional, default `false`)
-   `destroyOnHide` **[Boolean][75]** A boolean representing whether to destroy the modal state and unmount the modal after hide. (optional, default `true`)
-   `disableOutsideClick` **[Boolean][75]** A boolean representing whether clicking outside the modal div should hide the modal. (optional, default `false`)
...
Uses the lodash [omit][76] function under the hood.
...
-   `setSortFunc`: a function for setting a custom [comparison function][77] that will be used in `sort`
...
-   `options` **[Object][72]?** Options for the HOC
    -   `options.camelize` **[Boolean][75]** Option to camelize query parameter keys. This is true by default (optional, default `true`)
...
For more information: [https://reacttraining.com/react-router/web/api/withRouter][78]
...
[74]: https://github.com/reactjs/react-modal

[75]: https://developer.mozilla.org/docs/Web/JavaScript/Reference/Global_Objects/Boolean

[76]: https://lodash.com/docs/4.17.4#omit

[77]: https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Array/sort#Description

[78]: https://reacttraining.com/react-router/web/api/withRouter
